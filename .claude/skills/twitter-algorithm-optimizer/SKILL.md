...
## Compliance Preamble (VoidAI Marketing)

All tweet optimization suggestions MUST comply with VoidAI's CLAUDE.md compliance rules:
- Never suggest language that implies guaranteed returns
- Never add "allocation," "airdrop," "borrow" (in DeFi context), or other prohibited terms
- All optimized tweets must still pass the queue-manager's 6-step compliance pipeline
- Optimization must preserve required disclaimers — do not remove or shorten them for engagement
- The algorithm optimization is advisory — final content always goes through /queue add
- Never use banned AI phrases listed in CLAUDE.md (e.g., "game-changer," "cutting-edge," "robust ecosystem")
